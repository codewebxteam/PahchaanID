...
import { adminGetHotelDetail, type HotelSummary } from "@/lib/admin-api";
...
export default function AdminHotelDetailPage() {
  ...
  useEffect(() => {
    ...
    if (!token || !id) return;
    adminGetHotelDetail(token, id as string)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 items-start">
        ...
        <div className="lg:col-span-2 space-y-8">
          <Tabs defaultValue="managers">
            <TabsList className="bg-transparent border-b border-slate-200 w-full rounded-none h-auto p-0 flex space-x-8">
              <TabsTrigger 
                value="managers" 
                className="rounded-none border-b-2 border-transparent data-[state=active]:border-emerald-600 data-[state=active]:bg-transparent data-[state=active]:text-emerald-600 text-sm font-bold text-slate-400 px-1 py-3 transition-all"
              >
                Property Staff List
              </TabsTrigger>
              <TabsTrigger 
            ...
            <TabsContent value="subscriptions" className="py-6 focus-visible:outline-none space-y-4">
               {hotel.subscriptions?.length === 0 ? (
                 ...
                 <div key={sub.id} className={cn(
                   ...
                   sub.isActive ? "border-emerald-200 bg-emerald-50/20" : "border-slate-200 bg-white"
                 )}>
                    <div className="flex items-start gap-4">
                       <div className={cn("h-10 w-10 flex items-center justify-center rounded-lg border", sub.isActive ? "bg-emerald-600 text-white border-emerald-600 shadow-sm" : "bg-slate-100 text-slate-400 border-slate-200")}>
                          <ClipboardList className="h-5 w-5" />
                       </div>
                       <div>
                          <div className="flex items-center gap-3">
                             <p className="font-bold text-slate-800">Standard Management Tier</p>
                             {sub.isActive && <span className="text-[9px] font-bold bg-emerald-600 text-white px-2 py-0.5 rounded uppercase tracking-widest">Active Plan</span>}
                          </div>
        ...
        <div className="space-y-6">
           ...
           <div className="bg-white border border-slate-200 rounded-2xl divide-y divide-slate-100 shadow-sm">
              ...
              <div className="px-6 py-6 bg-slate-50/50">
                 <div className="flex items-center justify-between mb-4">
                    ...
                    <span className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest">94 / 100</span>
                 </div>
           ...
           <div className="bg-slate-900 rounded-2xl p-6 text-white overflow-hidden relative shadow-lg">
              ...
              <h3 className="text-[10px] font-bold text-emerald-400 uppercase tracking-[0.2em] mb-4 relative z-10">Regional Profile</h3>
